document.addEventListener('DOMContentLoaded', () => {
    ...
    const LAST_MATCH_KEY = 'lastMatchTeamsData'; // Stores the last played teams to keep arrows after refresh
    ...
    function renderTable(groupKey) {
        ...
        // Calculate total matches played in this group
        const totalMatches = teamData.reduce((sum, t) => sum + (t.won + t.lost), 0) / 2;
        ...
        teamData.forEach((team, newIndex) => {
            ...
            if (showArrow) {
                if (oldIndex > newIndex) {
                    icon = "▲"; color = "green"; blinkClass = "arrow-blink";
                } else if (oldIndex < newIndex) {
                    icon = "▼"; color = "red"; blinkClass = "arrow-blink";
                }
            }
            ...
            // ⭐ Smooth slide animation [cite: 19]
    ...
    function populateTeamSelects(groupKey) {
        ...
        const setDropdown = (select, placeholder) => {
            ...
            teams.forEach(team => {
                ...
                if (played >= 14) { opt.disabled = true; opt.style.color = "gray"; }
    ...
    function handleFormSubmit(e) { [cite: 37-41]
        ...
        if (!winner || !loser || winner === loser) { alert("Select valid teams."); return; }
        if (lostRounds < 0 || lostRounds > 7) { alert("Lost rounds must be 0–7."); return; }

        // Track last played teams for arrow logic
    ...
    function populateTieBreakerTeams() {
        ...
        teams.forEach(team => {
            ...
            if ((team.won + team.lost) >= 14) {
                opt1.disabled = opt2.disabled = true;
                opt1.style.color = opt2.style.color = "gray";
            }
            t1.appendChild(opt1);
            t2.appendChild(opt2);
        });
    ...
    function applyTieBreaker(groupKey, team1, team2, penalty) {
        ...
        lastMatchTeams[groupKey] = [team1, team2]; // Count as last match for arrows
    ...
    document.getElementById("tieApplyBtn").addEventListener("click", () => {
        ...
        if (!t1 || !t2 || t1 === t2) { alert("Select valid, different teams."); return; }
